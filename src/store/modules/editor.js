...
const actions = {
  ...
  copyPage({ commit }, uuid) {
    const pageData = state.projectData.pages.find(v => {
      return v.uuid === uuid
    })
  ...
  addElement({ commit }, elData) {
    ...
    // const data = editorProjectConfig.getElementConfig(elData, { zIndex: activePage.elements.length + 1 }) 重写
    let zIndex = activePage.elements.length + 1
    // 如果是背景、图层降到0，在所有元素下方
    if (elData.isBackground) {
      zIndex = 0
    }
    const data = editorProjectConfig.getElementConfig(elData, { zIndex: zIndex })
    commit('addElement', data)
    commit('setActiveElementUUID', data.uuid)
    commit('addHistoryCache')
  },
  removeBackgroundMap({ state, commit }) {
    state.projectData.bgMapObj = {}
  },
  addBackgroundMap({ state, commit }, elData) {
    const data = editorProjectConfig.getElementConfig(elData, { zIndex: -9999 })
    state.projectData.bgMapObj = data
    commit('addHistoryCache')
  },
  /**
...
const mutations = {
  ...
  deleteElement(state, uuid) {
    const activePage = getters.activePage(state)
    const elementIndex = activePage.elements.findIndex(v => {
      return v.uuid === uuid
    })
  ...
  resetElementZIndex(state, { uuid, type }) {
    ...
    const activePage = getters.activePage(state)
    const currentElement = activePage.elements.find(v => {
      return v.uuid === uuid
    })
...
const getters = {
  ...
  activeElement() {
    ...
    // debugger
    // 选择背景
    if (state.projectData.isBgMapOpen && state.projectData.bgMapObj.uuid === state.activeElementUUID) {
      return state.projectData.bgMapObj
    }
    // 选择组件
